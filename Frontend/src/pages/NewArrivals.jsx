...
export default function NewArrivals() {
  ...
  // Best seller IDs for the active gender (to exclude from new arrivals)
  const genderBestSellerIds = useMemo(() => {
    const ids = getBestSellers(genderCatalog, { limit: 8 }).map((p) => p.id);
    return ids;
  }, [genderCatalog]);

  // Color options from current gender catalog
  ...
  // Size options from current gender catalog
  ...
  // Filter + sort pipeline (applies on whole gender catalog, sorted by newest first by default)
  const filteredProducts = useMemo(() => {
    // Exclude best sellers from new arrivals (unless no other products available)
    const bestSellerSet = new Set(genderBestSellerIds);

    let list = genderCatalog.filter((p) => {
      // Prefer to exclude best sellers; only include if that would leave fewer than 4
    ...
    // Try to exclude best sellers; fall back to including them if result < 4
    const withoutBestSellers = list.filter((p) => !bestSellerSet.has(p.id));
    if (withoutBestSellers.length >= 4) {
      list = withoutBestSellers;
    }

    // Sort
    switch (sortBy) {
      case "newest":
        // Sort by isNew flag first, then by rating as tie-breaker
        list = [...list].sort((a, b) => {
  ...
  return (
    <div className="na-page">
      {/* Page Hero Banner */}
      <section className="na-hero" aria-label="New Arrivals Collection">
        <div className="container na-hero-inner">
          <span className="section-eyebrow">JUST RELEASED 2026</span>
          ...
          <div className="bs-gender-tabs" role="tablist" aria-label="Filter by category">
            {GENDER_TABS.map((g) => (
              <button
                ...
                type="button"
                role="tab"
                className={`bs-gender-tab na-gender-tab ${activeGender === g ? "active" : ""}`}
                onClick={() => setActiveGender(g)}
      ...
      {/* Catalog Section */}
      <section className="section-padding na-catalog-section">
          ...
          <div className="m-collection-toolbar">
            ...
            <div className="m-toolbar-right">
              ...
              <div className="m-sort-wrapper">
                <label htmlFor="na-sort-select" className="m-sort-label">Sort by:</label>
                ...
                <>
                  ...
                  <div className="m-pagination-wrap">
                    ...
                    {displayedProducts.length < filteredProducts.length ? (
                      ...
                      <p className="m-pagination-end">You've seen all {filteredProducts.length} new arrivals.</p>
                    )}
                  </div>
                </>
              ) : (
