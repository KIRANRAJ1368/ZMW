...
export default function Kids() {
  ...
  const priceCeiling = useMemo(
    () => Math.ceil(Math.max(...KIDS_PRODUCTS.map((p) => p.price * rate)) / 100) * 100,
    [rate]
  );

  // Filter States
  const [activeSubCategory, setActiveSubCategory] = useState(
    categoryFromUrl && KIDS_SUBCATEGORIES.includes(categoryFromUrl) ? categoryFromUrl : "All"
  );
  ...
  useEffect(() => {
    if (categoryFromUrl && KIDS_SUBCATEGORIES.includes(categoryFromUrl)) {
  ...
  const sizeOptions = useMemo(() => {
    const set = new Set();
    KIDS_PRODUCTS.forEach((p) => (p.sizes || []).forEach((s) => set.add(s)));
    return Array.from(set).sort((a, b) => {
      const order = ["0-1Y", "1-2Y", "2-3Y", "3-4Y", "4-5Y", "5-6Y", "6-7Y", "7-8Y", "8-9Y", "9-10Y"];
  ...
  const inStockCount = useMemo(
    () => KIDS_PRODUCTS.filter((p) => p.inStock).length,
    []
  );
  const outOfStockCount = KIDS_PRODUCTS.length - inStockCount;
  ...
  const filteredProducts = useMemo(() => {
    let list = KIDS_PRODUCTS.filter((p) => {
        ...
        {openFilters.category && (
          <div className="filter-accordion-content">
            <ul className="filter-category-list">
              {["All", ...KIDS_SUBCATEGORIES].map((cat) => {
                const count = cat === "All"
                  ? KIDS_PRODUCTS.length
                  : KIDS_PRODUCTS.filter((p) => p.subCategory === cat).length;
                return (
                  <li key={cat}>
                    <button
                      type="button"
  ...
  return (
    <div className="kids-collection-page">
      {/* ── Category Hero Banner — Premium Single Banner with Cute Kid Model ── */}
      {!hasCategorySelection && <section className="hero-section hero-single-banner kids-hero" aria-label="Kids' Collection Banner">
          ...
          <div className="hero-content hero-content-box">
            ...
            <div className="hero-campaign-info">
              ...
              <p className="hero-subtitle">
                Super-soft 100% organic combed cotton, hypoallergenic prints, and all-day playground durability.
              </p>
            </div>
            ...
            <div className="hero-perks-bar">
              <div className="hero-perk-item">
                ...
                <span className="hero-perk-label">Super-Soft Organic</span>
              </div>
              <div className="hero-perk-item">
                <span className="hero-perk-icon">
                ...
                <span className="hero-perk-label">4.9★ Parent Rating</span>
              </div>
              <div className="hero-perk-item">
                <span className="hero-perk-icon">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <span className="hero-perk-label">Fade-Resistant Colors</span>
              </div>
            </div>

            {/* 3. CALL TO ACTION & URGENCY PILL */}
            <div className="hero-cta-group">
              <a href="#kids-catalog" className="hero-btn-primary">
      ...
      <section id="kids-catalog" className="section-padding m-catalog-section">
      ...
      <section className="section-padding m-editorial-section">
        <div className="container">
          <div className="section-header">
            <span className="section-eyebrow">COMFORT & SAFETY</span>
            <h2 className="section-title">What Makes Our Kids' Edit Different?</h2>
            <p className="section-subtitle">
              Every piece is engineered from the ground up for soft handfeel, room to move, and
              prints that hold their color through the roughest playdates.
            </p>
          </div>
          ...
          <div className="m-editorial-grid">
            <div className="m-editorial-card">
              ...
              <h3 className="m-card-title">Skin-Friendly Soft Cotton</h3>
              <p className="m-card-desc">
                Woven from soft-washed, pre-shrunk cotton yarns that are gentle on sensitive skin.
                Holds its shape and color through repeated washing without stiffening.
              </p>
            </div>
            ...
            <div className="m-editorial-card">
              ...
              <h3 className="m-card-title">Room To Run, Jump & Play</h3>
              <p className="m-card-desc">
                Cut with extra ease through the shoulders and knees, with flat-locked seams that
                won't rub, so every piece keeps up with an active day.
              </p>
            </div>
            ...
            <div className="m-editorial-card">
              ...
              <h3 className="m-card-title">Fade-Proof, Skin-Safe Prints</h3>
              <p className="m-card-desc">
                Skin-safe inks and colorfast dye processes cured at precise temperatures. Won't
