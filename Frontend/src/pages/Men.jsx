...
import { MEN_PRODUCTS, MEN_SUBCATEGORIES } from "../data/products";
...
export default function Men() {
  ...
  const [activeSubCategory, setActiveSubCategory] = useState(
    categoryFromUrl && MEN_SUBCATEGORIES.includes(categoryFromUrl) ? categoryFromUrl : "All"
  );
  ...
  useEffect(() => {
    if (categoryFromUrl && MEN_SUBCATEGORIES.includes(categoryFromUrl)) {
  ...
  const sizeOptions = useMemo(() => {
    const set = new Set();
    MEN_PRODUCTS.forEach((p) => (p.sizes || []).forEach((s) => set.add(s)));
  ...
  const inStockCount = useMemo(
    () => MEN_PRODUCTS.filter((p) => p.inStock).length,
    []
  );
  const outOfStockCount = MEN_PRODUCTS.length - inStockCount;
  ...
  const filteredProducts = useMemo(() => {
    let list = MEN_PRODUCTS.filter((p) => {
        ...
        {openFilters.category && (
          <div className="filter-accordion-content">
            <ul className="filter-category-list">
              {["All", ...MEN_SUBCATEGORIES].map((cat) => {
                const count = cat === "All"
                  ? MEN_PRODUCTS.length
                  : MEN_PRODUCTS.filter((p) => p.subCategory === cat).length;
                return (
                  <li key={cat}>
                    <button
                      type="button"
  ...
  return (
    <div className="men-collection-page">
      {/* ── Category Hero Banner — Premium Single Banner with Modern Male Model ── */}
      {!hasCategorySelection && <section className="hero-section hero-single-banner men-hero" aria-label="Men's Collection Banner">
      ...
      <section id="men-catalog" className="section-padding m-catalog-section">
      ...
      <section className="section-padding m-recently-viewed-section">
        <div className="container">
          <div className="section-header">
            ...
            <p className="section-subtitle">
              Complementary heavyweight silhouettes and streetwear graphics chosen for your style.
            </p>
          </div>
      ...
      {/* Streetwear Quality Guide & Editorial Section */}
      <section className="section-padding m-editorial-section">
        <div className="container">
          <div className="section-header">
            <span className="section-eyebrow">CRAFTSMANSHIP & FIT</span>
            <h2 className="section-title">What Makes an Oversized T-Shirt Special?</h2>
            <p className="section-subtitle">
              Our streetwear tees are engineered from the ground up for authentic boxy drape, dense cotton
              weight, and enduring print fidelity.
            </p>
          </div>
          ...
          <div className="m-editorial-grid">
            <div className="m-editorial-card">
              ...
              <h3 className="m-card-title">240 GSM Combed Cotton</h3>
              <p className="m-card-desc">
                Woven from long-staple combed cotton yarns with double-knit density. Resists shrinkage, retains
                weight through multiple washes, and drapes cleanly with zero see-through.
              </p>
            </div>
            ...
            <div className="m-editorial-card">
              ...
              <h3 className="m-card-title">Engineered Drop-Shoulder Fit</h3>
              <p className="m-card-desc">
                Intentionally wider across the shoulders and chest with a balanced hemline. It provides an
                effortless, relaxed streetwear silhouette without looking overly sloppy.
              </p>
            </div>
            ...
            <div className="m-editorial-card">
              ...
              <h3 className="m-card-title">Fade-Proof Screen Prints</h3>
              <p className="m-card-desc">
                High-density plastisol and discharge inks cured at precise temperatures. Won’t crack, peel,
